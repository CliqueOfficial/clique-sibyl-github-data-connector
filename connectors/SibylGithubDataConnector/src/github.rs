    ...
    fn query(&self, query_type: &Value, query_param: &Value) -> Result<Value, String> {
        ...
        match query_type_str {
            ...
            "github_get_ecdsa_public_key" => {
                ...
                let res = simple_tls_client_no_cert_check(SIGN_CLAIM_SGX_HOST, &req, 12341).unwrap_or(json!({"result": {}}));
                let body: &Value = &res["result"];
                let result = json!({
                    "ecdsaPubkey": body["ecdsaPubkey"].as_str().unwrap_or(""),
                    "attestation": body["attestation"].as_str().unwrap_or("")
                });
                Ok(json!({
                    "result": result,
                    "reason": reason
                ...
                match parse_result(&plaintext) {
                    Ok(resp_json) => {
                        result = match panic::catch_unwind(|| {
                            ...
                            let req = format!(
                                "GET /ecdsaSign?data0={}&data1={}&data2={}&data3={}&data4={}&data5={} HTTP/1.1\r\n\
                                HOST: {}\r\n\
                                User-Agent: curl/7.79.1\r\n\
                                Accept: */*\r\n\r\n",
                            ...
                            let resp = simple_tls_client_no_cert_check(SIGN_CLAIM_SGX_HOST, &req, 12341).unwrap_or(json!({"result": {}}));
                            let ecdsa_sign: &Value = &resp["result"];
                            ...
                            return json!({
                                ...
                                "zk_claim": {
                                    "encryptedClaim": claim_str,
                                    "sign": ecdsa_sign["signature"].as_str().unwrap_or(""),
                                    "attestation": ecdsa_sign["attestation"].as_str().unwrap_or(""),
                                }
                            });
                        }) {
                            Ok(r) => r,
